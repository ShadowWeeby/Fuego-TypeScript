...
export default class Stats extends Command {
  ...
  description = 'Shows general statistics';
  ...
  execute = async (client: ExtendedClient, ctx: Context) => {
    ...
    const data = await client.cluster.broadcastEval(async (client: ExtendedClient) => {
      ...
      const stats =
        `${client.emoji.info} **ShardIds : ** [${[...client.ws.shards].map(([id]) => id).join(', ')}]\n` +
    ...
    await waitEmbed.edit({
      embeds: [
        client
          .embed()
          .desc(
            `${client.emoji.check} **Statistics**\n\n` +
              data
                .map(
                  (statData, index) => `${client.emoji.check} **Cluster ${index}**\n\n${statData}`,
                )
                .join('\n\n'),
          ),
      ],
