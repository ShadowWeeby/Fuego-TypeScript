...
export default class ContextCreate {
    constructor() {
        ...
        this.execute = async (client, ctx) => {
            ...
            if (bl)
                return;
            if (!(await resolvePerms.basic(ctx)))
                return;
            ...
            if (!command)
                return;
            if (!botAdmin && (await isUnderCooldown(ctx, command)))
                return;
            if (!(await enforceAdmin(ctx)))
                return;
            if (!(await resolvePerms.user(ctx, command, botAdmin)))
                return;
            if (!(await resolveBotAdmin(ctx, command)))
                return;
            if (client.underMaintenance && !botAdmin)
                return void client.emit('underMaintenance', ctx);
            if (args[0]?.toLowerCase() === '-h')
                return void client.emit('infoRequested', ctx, command);
            if (!(await resolveVoice(ctx, command)))
                return;
            if (!(await resolvePlayer(ctx, command)))
                return;
            if (!(await resolveNsfw(ctx, command)))
                return;
